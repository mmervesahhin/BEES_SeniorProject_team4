...
class MessageController {
  ...
  Future<void> sendMessage({
    ...
    Map<String, dynamic> entityMap =
        entity is Map<String, dynamic> ? entity : entity.toMap();
    ...
    if (receiverId != currentUserID) {
      String notificationMessage = content.trim().isNotEmpty
  ...
  Stream<QuerySnapshot> getMessages(
      String itemReqId, String userId, String otherUserId) {
  ...
  Stream<QuerySnapshot> getMessagesWithStatus(
    ...
    return _firestore
        ...
        .where('senderId',
            isNotEqualTo: currentUserId) // userId dışında olanlar
  ...
  Stream<QuerySnapshot> getMessagesWithStatuswithChatRoom(
  ...
  Stream<int> getSentMessagesCount(
      String chatRoomId, String currentUserId, String receiverId) {
    return _firestore
        .collection('chatRooms')
        .doc(chatRoomId)
        .collection('messages')
  ...
  Stream<int> getTotalUnreadMessagesCount(String currentUserId) {
    return FirebaseFirestore.instance
      ...
      for (var doc in snapshot.docs) {
        ...
        String entityStatus =
            await _getEntityStatus(chatRoom.entityType, chatRoom.entity);

        // Sadece aktif entity'ler için say
        if (entityStatus == 'active') {
          int unreadCount = await getSentMessagesCount(
                  chatRoom.chatRoomId, currentUserId, otherUserId)
              .first;
